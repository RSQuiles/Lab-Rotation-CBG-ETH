...
def run_model(adata, 
    ...
    if not os.path.exists(f"../results/models/{key}"):
        ...
        # Train model
        train_indices, val_indices = split_train_val(adata, 'cell_name') # Rafa: what should we have here as key??
        ...
        model.train(max_epochs=50, datasplitter_kwargs=datasplitter_kwargs, plan_kwargs=training_plan_kwargs, **early_stopping_kwargs) 
        end_time = time.time()
...
def compute_metrics(adata, key, model=None):
    ...
    for var in ['cell_name','cell_state','Response']:  
...
# adata = sc.read("/cluster/work/bewi/data/tahoe100/h5ad/controls_processed.h5ad") # processed controls
# adata = sc.read("/cluster/work/bewi/data/tahoe100/h5ad/controls_merged.h5ad") # original controls
# _data_path = "/cluster/work/bewi/members/rquiles/experiments/datasets/3_cells_2_drugs_balanced_counts.h5ad"
data_path = "/cluster/work/bewi/data/tahoe100/h5ad/gdsc_controls_processed_fcr.h5ad"
print(f"Loading AnnData from {data_path} at", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), flush=True)
adata = sc.read(data_path)

# Subset dataset speed up computation
rng = np.random.default_rng(42)
n_cells = adata.n_obs
sensible_size = 30_000
subset_size = sensible_size if n_cells > sensible_size else n_cells
...
# Added embeddings by Rafa: PCs, HVGs, FCR
# Note: It is assumed the AnnData already has embeddings precomputed, mostly in adata.obsm

# All keys
all_keys = []

# FCR Embeddings
print("Checking FCR embeddings at", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), flush=True)
if "ZXs" not in adata.obsm.keys():
    print("ZX embeddings not found in adata.obsm.", flush=True)
    exit(1)
if "ZXTs" not in adata.obsm.keys():
    print("ZXT embeddings not found in adata.obsm.", flush=True)
    exit(1)
if "ZTs" not in adata.obsm.keys():
    print("ZT embeddings not found in adata.obsm.", flush=True)
    exit(1)

fcr_keys = ["ZXs", "ZXTs", "ZTs"]

# PCs
print("Checking PCA embeddings at", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), flush=True)
pcs = [10, 30, 50]
max_pcs = max(pcs)
if "X_pca" not in adata.obsm.keys():
    print("PCs not found in adata.obsm", flush=True)
    exit(1)
elif adata.obsm["X_pca"].shape[1] < max_pcs:
    print("Input PC dimension larger than maximal precomputed dimension", flush=True)
    exit(1)
else:
    for n_pcs in pcs:
        adata.obsm[f"X_pca_{n_pcs}"] = adata.obsm["X_pca"][:, :n_pcs]

pcs_keys = [f"pca_{n}" for n in pcs]
all_keys.extend(pcs_keys)

# HVGs
print("Checking HVG embeddings at", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), flush=True)
# n_hvgs = [2000, 3000, 4000, 5000]
n_hvgs = [3000]
max_hvg = max(n_hvgs)

if "highly_variable" not in adata.var.columns:
    print("HVGs not found in adata.var", flush=True)
    exit(1)

if adata.var['highly_variable'].sum() < max_hvg:
    print("Input HVG dimension smaller than maximal precomputed dimension", flush=True)
    exit(1)

for n in n_hvgs:
    # genes included in this HVG definition
    selected = sorted_genes[:n]

    # boolean mask for selected genes
    gene_mask = adata.var_names.isin(selected)

    # extract expression matrix for selected genes
    X_hvg = adata.X[:, gene_mask]

    # ensure dense if needed
    if scipy.sparse.issparse(X_hvg):
        X_hvg = X_hvg.toarray()

    # store as an embedding vector
    adata.obsm[f"X_hvg_{n}"] = X_hvg

hvg_keys = [f"hvg_{n}" for n in n_hvgs]
all_keys.extend(hvg_keys)
...
# Combine all metrics (for scVI models)
...
for var in ['cell_name']:
    print("Running benchmark for variable:", var)
    bm = Benchmarker(
        adata,
        batch_key=BATCH_KEY,
        label_key=var,
