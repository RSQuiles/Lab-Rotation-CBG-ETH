...
def run_model(adata, 
    ...
    if not os.path.exists(f"../results/models/{key}/model.pt"):
        ...
        # n_categories = adata.obs['cell_state'].nunique()
        ...
        # n = adata.obs[f"gmm_{key}"].nunique()
        # adata.uns[f"gmm_{key}_colors"] = plt.get_cmap("tab20").colors[:n]
        # sc.settings.figdir = f"../results/figures/{key}/"
        # for feature in [f"leiden_{key}", f"gmm_{key}", CELL_TYPE_KEY, "cell_state", "Response", BATCH_KEY]:
        ...
        #     if f"gmm_cell_state_{key}" in adata.obs.keys():
        #         print(f"Using existing GMM labels for {key}", flush=True)
        #         adata.obs[f"gmm_{key}"] = adata.obs[f"gmm_cell_state_{key}"]
        #         n = adata.obs[f"gmm_{key}"].nunique()
        #         adata.uns[f"gmm_{key}_colors"] = plt.get_cmap("tab20").colors[:n]
        ...
        # compute_metrics(adata=adata, key=key, model=model)
...
masks_list = [masks_kegg, masks_kegg, masks_kegg_pathways, masks_kegg_rnd, masks_kegg_pathways_rnd]
keys_list = ["piscVI_kegg_tanh", "piscVI_kegg_nb_tanh", "piscVI_kegg_tanh_pathways", "piscVI_kegg_tanh_rnd", "piscVI_kegg_tanh_pathways_rnd"]
activations_list = ["tanh", "tanh", "tanh", "tanh", "tanh"]
likelihoods_list = ["zinb", "nb", "zinb", "zinb", "zinb"]
...
for masks, key in zip(masks_list, keys_list):
    ...
    adata_kegg.write("../results/models/piscVI_kegg.h5ad")
...
#         masks=masks,   
#     )
#     adata_kegg.obsm[f"X_{key}"] = adata_reactome.obsm[f"X_{key}"]
#     adata_kegg.write("../results/models/piscVI.h5ad")

# del adata_reactome

# # Test with more hvgs
# for db in ['Reactome']: #'KEGG_NB', 'KEGG']: #,
...
#             masks=masks,   
#         )
#         adata_kegg.obsm[f"X_{key}"] = adata_7k.obsm[f"X_{key}"]
#         adata_kegg.write("../results/models/piscVI.h5ad")
