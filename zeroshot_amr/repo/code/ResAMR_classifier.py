...
def main(args):
    ...
    # Read data
    print(f"Reading data from {args.driams_long_table}, {args.spectra_matrix}, and {args.drugs_df}...")
    ...
    # print("Long table: ", driams_long_table.head())
    ...
    print("Setting up model and experiment...")
    ...
    # Train model
    # if not args.test_only: # Added by Rafa
    print("Training...")
    trainer = pl.Trainer(devices="auto", 
                        accelerator="auto", 
                        default_root_dir=output_folder, 
                        max_epochs=args.n_epochs, 
                        # callbacks=callbacks, # Activated by Rafa
    ...
    # Test model
    print("Testing...")
    # Added by Rafa: load best checkpoint
    """
    checkpoint_dir = join(output_folder, "checkpoints")
    best_ckpt = checkpoint_callback.best_model_path
    if args.test_only:
        if not exists(best_ckpt):
            raise RuntimeError(f"No checkpoint found at {best_ckpt}. Cannot run --test_only.")
        experiment = Classifier_Experiment.load_from_checkpoint(best_ckpt, config=config, model=model)

    trainer = pl.Trainer(devices="auto", accelerator="auto")
    test_results = trainer.test(experiment, dataloaders=test_loader)
    """
    ...
    # Modified by Rafa: use Parquet files for large datasets
    test_df.to_csv(join(results_folder, f"test_set_seed{seed}.csv"), index=False)
    # test_df.to_parquet(join(results_folder, f"test_set_seed{seed}.parquet"), index=False)
...
if __name__=="__main__":
    ...
    # Added by Rafa: option to only test
    parser.add_argument("--test_only", action="store_true", help="Whether to only run testing using a pre-trained model")

    args = parser.parse_args()

    ### Modified: Rafa, set num_workers to number of allocated CPUs (SLURM)
